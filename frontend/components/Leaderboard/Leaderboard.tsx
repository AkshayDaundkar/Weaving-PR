        ...
        return (
          <li
            ...
            className={`
              ...
              ${isSelected ? "border-accent bg-accent/5 ring-1 ring-accent/30" : "border-bg-border bg-bg-primary hover:bg-bg-secondary"}
            `}
          >
            <div className="flex items-center gap-4">
              <span className="text-[var(--text-muted)] font-mono w-8 shrink-0">#{e.rank}</span>
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-0.5 truncate">{e.narrative}</p>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <div className="w-20 h-2 rounded-full bg-bg-tertiary overflow-hidden" title="Impact score">
                  <div
                    className={`h-full rounded-full ${scoreBarColor(e.impact_score)}`}
                    style={{ width: `${Math.min(100, Math.max(0, e.impact_score))}%` }}
                  />
                </div>
                <span className="font-semibold text-[var(--text-primary)] w-10 text-right">
                  {e.impact_score.toFixed(1)}
                </span>
                {e.momentum !== 0 && (
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded ${
                      e.momentum > 0 ? "bg-bg-tertiary text-momentum-up" : "bg-bg-tertiary text-momentum-down"
                    }`}
                  >
                    {e.momentum > 0 ? "+" : ""}
